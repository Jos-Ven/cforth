marker -rcvfile.fth  cr lastacf .name #19 to-column  .( 05-05-2025 ) \ By J.v.d.Ven
...
nuser vector-table   0 vector-table !
nuser &UdpData
...
#7 cells              constant /vector-table

$7f000000 constant first-vector       0 value last-vector
#30 constant /MaxFilenameLengthRcv  #48 constant >filenameRcv
#16 constant /flash-server          #30 constant /TcpBuffer
 #1 constant TcpTimeout               0 value fd-file
  0 value rcv-interactive?            0 value &UdpBuffer
  0 value &UdpPacket                  0 value &LastUdpByte
  0 value flash-server$               0 value &TcpBuffer

variable fsize         variable #retries
variable msStartFlash  variable #done
variable /UdpSize

nuser udp-socket         udp-socket off
variable tcp-server      tcp-server off

: allocate-udp-buffers   ( -- )
...
;
: AnswerWsPing ( - )
   &UdpPacket 40  s" wsping " dup >r search  if
      &TcpBuffer off s" -2130706460 PingReply " &TcpBuffer lplace
      ip"  &TcpBuffer +lplace
...
: start-flash  ( - )
   ...
   [char] t emit ms@ msStartFlash !
;
: start-flash-session ( - )
   &UdpData @  /num + /num bl GetValue ( Get the filesize )  if
      dup fsize ! set-flash-server
      CreateFileSpace  to fd-file ." e"
      prep-flash start-flash \ start a flash session
   else
      drop cr ." Invalid size." cr
   then
;
: .modk ( n div1000 - )
   dup 0=  if
...
: UdpLoad ( - )
   GetFilename  if
      2dup file-exist?  if
         SetAppFile bye \ Restarts Forth and compiles the startfile
...
: repair  ( - )     \ For dropped packets.
    fsize @ dup 0>   if
       /UdpData /mod swap  if  1+  then
       0  do
          i  /UdpData *  [ /UdpData 1 - ] literal + fsize @ 1- min  \ last byte in packet in file
          s>d fd-file reposition-file drop
          &TcpBuffer off fd-file file-size  1 ms 3drop
          &TcpBuffer 1 fd-file read-file 2drop &TcpBuffer c@ 0=   if \ find missing packets
             \ cr i cr ." 1>>" . true to dmp
             [char] r emit  \ retry
...
: exe-vector ( -vector|Unknown - )
   abs dup first-vector  last-vector between if \ See also UdpSender.f
      first-vector -  vector-table @ + @ execute
   else
      drop
   then
;
...
: StripOverSized  ( #towrite position-in-file - #towrite-new )
   s" /UdpData + fsize @ >  if  drop fsize @  /UdpData /mod drop  then"  evaluate
;  immediate
...
;
: receive-udp-packets ( -- )
    &UdpPacket off &UdpPacket /UdpPacket udp-socket @ lwip-read dup 0> if   ( - #read  )
         &UdpPacket /num bl GetValue              \ ( - #read positon|size|vector flag )
\   dmp if  cr .s ." --2>> " cr &UdpPacket 50 dump  then \ Needed here in case of trouble.
                                      if      \ ( - #read positon|size|vector  )
             dup 0>=  if                      \ A vector is negative ( - #read position  )
                 >r &UdpPacket swap DataOffset /string dup #done +! 0 max
                 r>  write-in-file blink exit \ Write the received packet at it's position
              else
                 swap /UdpSize !  exe-vector  \ Execute the vector
              then
...
: receiver-in-task ( -- )  \ To receive a file using the UDP protocol in a task.
   ...
   begin  receive-udp-packets  again
   end-task
;
...
: receiver ( -- )  \ To receive a file using the UDP protocol interactive.
    true to rcv-interactive?
       [ifndef]  spi_master_write64  cr bold ." Upload to: " get-path type norm cr [then]
    ipaddr@ @ 0=   if   logon   then
    upd-init receiver-msg rcv-prompt
    begin  receive-udp-packets  stop-receiver? key? or  until
    CloseRcv
;
alias r receiver
...
: /update ( - )
   s" /spiffs/_receiver_bg.txt" file-exist? if
      s" The receiver runs in the background. " 2dup
      cr type MsgPage
   else
      s" Waiting to receive a file from the upload server... " MsgPage
      SendHtmlPage 30 ms receiver
   then
;
